...
## Zones for screeps-ai-v03

When splitting parallel explore tasks for this repo, prefer:

- **Tick entry:** `src/index.ts` (role pass order, `moduleScope` wiring)
- **Roles:** `src/roles/*.ts`, `src/roles/fsm.ts`, `src/roles/energyAcquisition.ts`
- **Management:** `src/management/*.ts` (room cache, spawn, construction)
- **Types and contracts:** `src/types.d.ts`
- **Tooling and CI:** `package.json`, `scripts/`, `.github/workflows/`
